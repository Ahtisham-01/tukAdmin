...
const index = ({ setEditCompDetails, setAddComponent, subcategories, allComponents, setIsEdit }) => {
  const [del, setDel] = useState(false)
  const [modalObj, setModalObj] = useState(false)
  const date = new Date()
  console.log(subcategories, 'v')
  return (
      ...
      {false ? (
        ...
        <>
          <table className="w-full whitespace-nowrap ">
            {allComponents == undefined || allComponents?.length === 0 ? <thead>
              {/* {true ? <thead> */}
            ...
            <tbody>
              {allComponents !== undefined && allComponents?.length > 0 ? (
                // {false ? (
                allComponents?.slice().reverse().map((item, i) => {
                  return (
                    <tr
                      key={i}
                      // onClick={() => {
                    ...
                    >
                      ...
                      <td
                        className={`w-full max-w-[224px] min-w-[224px] text-start font-normal text-sm leading-[150%] px-[16px] `}
                      >
                        <img
                          src={
                            item?.componentsPicture
                          }
                          className="w-[75px] h-[25px]"
                        />
                      </td>
                      <td className="w-full max-w-[224px] min-w-[224px] text-start font-normal text-sm leading-[150%] px-[16px]  ">
                        {item?.subCategoryId === null ? 'N/A' : subcategories !== undefined && subcategories[i]?.name}
                      </td>
                      <td className="w-full max-w-[224px] min-w-[224px] text-start font-normal text-sm leading-[150%] px-[16px] ">
