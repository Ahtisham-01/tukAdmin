import { useCreateTemplatesMutation, useUpdateTemplatesMutation } from '../../../features/api/dataApiSlice';
import React, { useState, useEffect } from 'react'
import { useDispatch, useSelector } from 'react-redux';

const AddNewTemplate = ({ seteditTemplateDetails, editTemplateDetails, setaddTemplates, isEdit }) => {
    ...
    const create = (event) => {
        ...
        payload.append("templatesPicture", imgForm);
        payload.append("name", label);
        payload.append("description", description);
        createTemplate(payload).unwrap().then((res) => {
            console.log(res, 'res')
            setButton(false)
    ...
    useEffect(() => {
        if (Object.keys(editTemplateDetails).length > 0) {
            setLabel(editTemplateDetails?.name)
            setDescription(editTemplateDetails?.description)
        }
    ...
    const editComponents = async (e) => {
        ...
        updateTemplate({ id: editTemplateDetails?.id, payload }).then((res) => {
            setButton(false)
            console.log(res, 'res')
            setaddTemplates(false)
        }).catch((err) => {
            ...
            {isEdit ?
                <>
                    ...
                    <div className="flex justify-between py-8">
                        <p className="text-2xl font-extrabold leading-tight text-gray-800 w-2/3 capitalize">Edit Template</p>
                    </div>
                    <form onSubmit={editComponents} className="p-5 flex flex-col my-8 lg:my-0 xl:my-0 bg-white h-full">
                        ...
                        {Object.keys(editTemplateDetails).length === 0 ? <div
                            ...
                            <div className="h-32">
                                <img
                                    ...
                                    src={
                                        editTemplateDetails?.templatesPicture
                                    }
                                />
                            </div>
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            >
                                {(Object.keys(editTemplateDetails).length > 0 && "Update Template")}
                            </button>
                            <div
                                onClick={() => {
                                    setTimeout(() => {
                ...
                : <>
                    ...
                    <div className="flex justify-between py-8">
                        <p className="text-2xl font-extrabold leading-tight text-gray-800 w-2/3 capitalize">Create Template</p>
                    </div>
                    <form onSubmit={(event) => create(event)} className="p-5 flex flex-col my-8 lg:my-0 xl:my-0 bg-white h-full">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            >
                                {
                                    (Object.keys(editTemplateDetails).length === 0 && "Create Template")
                                }
                            </button>
                            <div
                                onClick={() => {
