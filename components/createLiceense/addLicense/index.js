...
import { useCreateLicenseMutation, useUpdateLicenseMutation } from "../../../features/api/dataApiSlice"

function Index({ setaddLicense, addLicense,  customerData, setLicenseItem, licenseItem })
 {
    // license info states
    ...
    useEffect(() => {
        if (licenseItem) {
            ...
            setprice(licenseItem?.price)
            setdescription(licenseItem?.description)
            setMemberId(licenseItem?.id)
        } else {
            setLicenseItem("")
        }
    }, [licenseItem?.id])
    ...
    return (
        <>
            <main className="bg-white flex xl:flex-nowrap flex-col  flex-wrap pt-0 pb-7 gap-8">
                ...
                <form
                    onSubmit={(e) => {
                        !licenseItem ? submitHandler(e) : updateHandler(e)
                    }}
                >
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        >
                            {loader ? (
                                ...
                                (!licenseItem && "Create License") ||
                                (licenseItem && "Update License")
                            )}
                        </button>
                        <div
                            onClick={() => {
