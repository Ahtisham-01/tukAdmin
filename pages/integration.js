...
// import AddIntegrationData from '../components/createIntegration/addIntegration'
const AddIntegrationData = dynamic(() => import('../components/createIntegration/addIntegration'), {
    ssr: false,
})
import userAccess from "../helper/access"
import BlackSpinner from '../components/reusableUi/blackSpinner'
import { useGetAllComponentsQuery, useGetAllIntegrationQuery } from '../features/api/dataApiSlice'
...
const Integration = () => {
    ...
    const { data: allIntegrations, isLoading, isFetching } = useGetAllIntegrationQuery()
    ...
    useEffect(() => {
        if (textSearch !== '') {
            const filterData = allIntegrations?.filter((item) => item?.framework?.toLowerCase().includes(textSearch?.toLowerCase()))
            setFilterValue(filterData)
        } else {
            setFilterValue(allIntegrations)
        }
    }, [textSearch, isLoading, isFetching,])

    return (
        <div>
            <Head>
                <title>TUK Create Templates</title>
                <meta
                    name="viewport"
                    ...
                    <Layout nav={true}>
                        ...
                        <div className="max-w-[1124px] w-full container mx-auto">
                            ...
                            <IntegrationData
                                ...
                                setIntegrationItem={setIntegrationItem}
                        ...
                        <>
                            ...
                            <div
                                className={`bg-white fixed inset-0 h-full max-w-[384px] transition-all duration-700 ease-in-out z-[500] m-auto ${addIntegration ? "mr-0" : "mr-[-100%]"
