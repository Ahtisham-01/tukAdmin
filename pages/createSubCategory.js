...
import AddSubCategory from '../components/createSubCategory/addSubCategory'
import Head from 'next/head'
import { useGetAllCategoryQuery, useGetSubcategoriesQuery } from '../features/api/dataApiSlice'
const createSubCategory = () => {
  ...
  const { data: allSubcategory, isLoading } = useGetSubcategoriesQuery()
  ...
  useEffect(() => {
    if (textSearch?.length > 0) {
      const filterData = allSubcategory?.filter((item) => item?.name?.toLowerCase().includes(textSearch?.toLowerCase()))
  ...
  return (
    <div>
      <Head>
        <title>TUK Create SubCategory</title>
      </Head>
      {
        loading ? (
          ...
          <Layout nav={true}>
            ...
            <>
              ...
              <div
                className={`bg-white fixed inset-0 h-full max-w-[384px] transition-all duration-700 ease-in-out z-[500] m-auto ${addSubCategory ? "mr-0" : "mr-[-100%]"
