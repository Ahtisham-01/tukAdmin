...
import AddComponents from '../components/createComponents/addcomponent'
import { useDispatch, useSelector } from 'react-redux'
import { useGetAllComponentsQuery, useGetSubcategoriesQuery } from '../features/api/dataApiSlice'
...
const createComponent = () => {
    ...
    const [editCompDetails, setEditCompDetails] = useState({})
    ...
    const { data: allComponent, isLoading } = useGetAllComponentsQuery()
    useEffect(() => {
        if (textSearch?.length > 0) {
            const filterData = allComponent?.filter((item) => item?.name?.toLowerCase().includes(textSearch?.toLowerCase()))
            if (filterData && filterData !== undefined) {
                setAllComponents([...filterData])
            }
        } else {
            setAllComponents(allComponent)
        }
    }, [textSearch, isLoading, allComponent?.length])
    return (
        <div>
            <Head>
                <title>TUK Create Components</title>
                <meta
                    name="viewport"
                    ...
                    <>
                        ...
                        <div className="max-w-[1124px] w-full container mx-auto">
                            ...
                            <CreateComponents
                                ...
                                setEditCompDetails={setEditCompDetails}
                                setIsEdit={setIsEdit}
                        ...
                        <>
                            ...
                            <div
                                className={`bg-white fixed inset-0 h-full max-w-[684px] transition-all duration-700 ease-in-out z-[500] m-auto ${addComponent ? "mr-0" : "mr-[-100%]"
                                    } w-full p-6 min-h-screen`}
                            >
                                {/* <addComponent
                                            setaddComponent={setaddComponent}
                              ...
                              <AddComponents
                                isEdit={isEdit}
                                setAddComponent={setAddComponent}
                                editCompDetails={editCompDetails}
                                addComponent={addComponent}
                                subcategories={subcategories}
                                setEditCompDetails={setEditCompDetails}
                            />
