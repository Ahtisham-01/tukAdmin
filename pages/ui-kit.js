...
import AddUiKit from '../components/createUiKit/addNewKit'
import CreateUiKit from '../components/createUiKit'
import { useDispatch, useSelector } from 'react-redux'
import { useGetAllLicenseQuery, useGetAllUiKitsQuery } from '../features/api/dataApiSlice'
...
const UiKit = () => {
    ...
    const [addUIKit, setaddUIKit] = useState(false)
    const [uiKitItem, setUiKitItem] = useState({})
    const [allKits, setAllKits] = useState([])
    const { data: allUIKits, isLoading } = useGetAllUiKitsQuery()
    const { data: allLicense } = useGetAllLicenseQuery()

    useEffect(() => {
        if (textSearch?.length > 0) {
            const filterData = allUIKits?.filter((item) => item?.name?.toLowerCase().includes(textSearch?.toLowerCase()))
            if (filterData && filterData !== undefined) {
                setAllKits([...filterData])
            }
        } else {
            setAllKits(allUIKits)
        }
    }, [textSearch, isLoading])
    return (
        <div>
            <Head>
                <title>TUK Create UIKit</title>
                <meta
                    name="viewport"
            ...
            <Layout nav={true}>
                ...
                <>
                    ...
                    <div className="max-w-[1124px] w-full container mx-auto">
                        ...
                        <CreateUiKit
                            setUiKit={setUiKitItem}
                            setaddUIKit={setaddUIKit}
                            allKits={allKits}
                    ...
                    <>
                        ...
                        <div
                            className={`bg-white fixed inset-0 h-full max-w-[384px] transition-all duration-700 ease-in-out z-[500] m-auto ${addUIKit ? "mr-0" : "mr-[-100%]"
