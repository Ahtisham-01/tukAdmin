...
import AddLicensesToUser from '../../../components/CreateUserLicense/addLicensesToUser'
import Head from 'next/head'
import { useGetAllUsersQuery } from '../../../features/api/dataApiSlice'
import CreateUserLicense from '../../../components/CreateUserLicense'
...
const index = () => {
    ...
    useEffect(() => {
        if (textSearch?.length > 0) {
            const filterData = users?.filter((item) => item?.name?.toLowerCase().includes(textSearch?.toLowerCase()))
            setAllUsers(filterData)
        } else {
            setAllUsers(users)
        }
    }, [textSearch, isLoading, users?.length])

    return (
        <div>
            <Head>
                <title>TUK Users</title>
                <meta
                    name="viewport"
            ...
            <Layout nav={true}>
                ...
                <>
                    ...
                    <div
                        className={`bg-white fixed inset-0 h-full max-w-[384px] transition-all duration-700 ease-in-out z-[500] m-auto ${addUsers ? "mr-0" : "mr-[-100%]"
