...
import SupportAll from '../components/SupportAll'
import AddSuppport from '../components/SupportAll/addSupport'
import TabControler from '../components/SupportAll/tabControle'
import { useGetallTicketsQuery, useGetAllTicketQuery } from '../features/api/dataApiSlice'
...
const Support = () => {
    ...
    const { data: allTickets, isLoading, isFetching } = useGetAllTicketQuery()
    ...
    const filterDataTicket = () => {
        if (ticketType == 0) {
            setFilterValue(allTickets)
        }
        else if (ticketType == 1) {
            const resolved = allTickets.filter((item) => {
                if (item.status === "resolved") return item
            })
            setFilterValue(resolved)
        }
        else if (ticketType == 2) {
            const open = allTickets.filter((item) => {
    ...
    useEffect(() => {
        if (textSearch?.length > 0) {
            const filterData = allTickets?.filter((item) => item?.subject?.toLowerCase().includes(textSearch?.toLowerCase()))
            setFilterValue(filterData)
        } else {
            filterDataTicket()
        }
    }, [textSearch, isLoading, ticketType, isFetching])

    return (
        <div>
            <Head>
                <title>TUK Support</title>
                <meta
                    name="viewport"
                ...
                loading ? (
                    ...
                    <Layout nav={true}>
                        ...
                        <div className="max-w-[1124px] w-full container mx-auto">
                            ...
                            <div className="w-full flex justify-between items-center mt-8 mb-[17.5px]">
                        ...
                        <>
                            ...
                            <div
                                className={`bg-white fixed inset-0 h-full max-w-[384px] transition-all duration-700 ease-in-out z-[500] m-auto ${addSupport ? "mr-0" : "mr-[-100%]"
