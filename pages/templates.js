...
import AddNewTemplate from '../components/createTemplates/addNewTemplate'
import { useGetAllTemplatesQuery } from '../features/api/dataApiSlice'
...
const Templates = () => {
    ...
    const [editTemplateDetails, seteditTemplateDetails] = useState({})
    const [allTemplates, setAllTemplates] = useState([])
    const { data: allTemplate, isLoading, isFetching } = useGetAllTemplatesQuery()
    ...
    useEffect(() => {
        if (textSearch?.length > 0) {
            const filterData = allTemplate?.filter((item) => item?.name?.toLowerCase().includes(textSearch?.toLowerCase()))
            if (filterData && filterData !== undefined) {
                setAllTemplates([...filterData])
            }
        } else {
                        ...
                        <>
                            ...
                            <div className="max-w-[1124px] w-full container mx-auto">
                                ...
                                <CreateTemplates
                                    ...
                                    seteditTemplateDetails={seteditTemplateDetails}
                                    setIsEdit={setIsEdit}
                            ...
                            <>
                                ...
                                <div
                                    className={`bg-white fixed inset-0 h-full max-w-[684px] transition-all duration-700 ease-in-out z-[500] m-auto ${addTemplates ? "mr-0" : "mr-[-100%]"
